...
void main() {
  test('csv exporter build section lengkap', () {
    ...
    final String csv = const TrendCsvExporter().buildCsv(
      dashboard,
      generatedAt: DateTime(2026, 5, 6, 10, 30),
    );

    expect(csv, contains('"Lariss Trend Export"'));
    expect(csv, contains('"Revenue trend"'));
    expect(csv, contains('"Jam ramai"'));
    expect(csv, contains('"Produk turun"'));
    expect(csv, contains('"Insights"'));
  });
}
